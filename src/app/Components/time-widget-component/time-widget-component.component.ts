...
@Component({
  ...
  templateUrl: './time-widget-component.component.html',
  styleUrls: ['./time-widget-component.component.scss'],
})
export class TimeWidgetComponent implements OnInit {
  ...
  getTimeData(value: string){
    ...
    if(value==="First"){
      call='https://api.bigdatacloud.net/data/timezone-by-ip?utcReference=0&key=cb435ebd3bd7445885588784710c5cf4'
    }
    else{
      value = value.replace(/\//gi, "%2F");
      call='https://api.bigdatacloud.net/data/timezone-info?timeZoneId='+value+'&utcReference=0&key=cb435ebd3bd7445885588784710c5cf4'
    }
    ...
    .then((data:IApiResponse)=>{
      ...
      this.TimeData.place=data.ianaTimeId.split("/", 2)[1];
      if(!this.firstSec){
